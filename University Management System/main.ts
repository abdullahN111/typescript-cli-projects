...
class Student extends Person {
  ...
  toString(): string {
    return `Student { Name: ${this.name}, Age: ${this.age}, Student #id: ${
      this.rollNumber
    }, Course: [${this.courses
      .map(
        (course) =>
          `${course.name} (Instructor: ${course.instructors
            .map((inst) => inst.name)
            .join(", ")})`
      )
      .join(", ")}] }`;
...
class Instructor extends Person {
  ...
  toString(): string {
    return `Instructor { Name: ${this.name}, Age: ${this.age}, Salary: ${
      this.salary
    }, courses: [${this.courses.map((course) => course.name).join(", ")}] }`;
  }
}
  ...
  const {
    ...
    },
    {
      ...
      choices: function (answers) {
        ...
        const instt = courses.find(
          (i) => i.instructors === answers.instructorName
        );
